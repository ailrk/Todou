...
export function fmtYM(ymd: YMD): string {
  return `${ymd.year}-${String(ymd.month + 1).padStart(2, '0')}`
}


export function fmtYMD(ymd: YMD): string {
...
export class PresenceView {
  ...
  view(i: number, calendar: YMD, firstDay?: string): { presence: boolean, completed: boolean } {
...
export async function base64ToBitSet(b64: string): Promise<PresenceView> {
...
export async function zlibDecompress(blob: Blob) {
